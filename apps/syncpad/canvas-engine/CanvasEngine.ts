...
export class CanvasEngine {
  ...
  private handleTexty(e: MouseEvent) {
    ...
    textarea.classList.add("syncpad-texty");
    ...
    const syncpadContainer = document.querySelector(
      ".syncpad-textEditorContainer"
    );

    if (syncpadContainer) {
      syncpadContainer.appendChild(textarea);
    ...
    const save = () => {
      ...
      if (syncpadContainer?.contains(textarea)) {
        syncpadContainer.removeChild(textarea);
        if (span && document.body.contains(span)) {
          document.body.removeChild(span);
        }
      }

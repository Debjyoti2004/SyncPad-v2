...
export function AppSidebar({ isOpen, onClose, canvasColor, setCanvasColor, isMobile, roomName, isStandalone, onClearCanvas, onExportCanvas, onImportCanvas }: SidebarProps) {
    ...
    useEffect(() => {
        const fetchRepoMetaData = async () => {
            try {
                const res = await fetch('https://github.com/Debjyoti2004/SyncPad-v2.git');
                    ...
                    <div className={cn("py-1", isMobile ? "" : "flex-1 overflow-auto py-1 custom-scrollbar")}>
                        <nav className={cn("grid gap-1", isMobile ? "px-0" : "px-2")}>
                            ...
                            <Link
                                ...
                                href="https://github.com/Debjyoti2004/SyncPad-v2.git"
                                target="_blank"
                            ...
                            <SidebarLinkItem icon={Twitter} label="Twitter / X" url="https://x.com/DebjyotiSh27921" />
                            <SidebarLinkItem icon={Linkedin} label="Linkedin" url="https://www.linkedin.com/in/debjyotishit/" />
                        </nav>
                    </div>
